...
def adjustP(pvalues, method = "Benjamini-Hochberg"):
    ...
    if method == "Bonferroni":
        cp = list(map(lambda x:min(x*n,1.0), pvalues))
...
def writeRecode(infile, digits, test, model):
    ...
    header = list(map(lambda i: i.translate(tab), header))
    f.write('\t'.join(header))
    f.write('\n')
    f.flush()
    for i in ans:
...
def regressionLogistic(infile, digits, freq, model = 'additive', adjust = 'FDR', exclude=None, covfile=None, covname=None, perm=None, seed=None, test='logistic'):
    ...
    geno = pd.read_csv(tfile,sep=r'\s+', engine='python', header=0)
    os.remove(tfile)
    ...
    if covfile is not None:
        cov = pd.read_csv(covfile,sep=r'\s+', engine='python', header=0)
        ...
        if allele not in excludeAlleles:
            ...
            if f12 > freq:
                ...
                try:
                    ...
                    p = lr.pvalues.iloc[1]
                    try:
                        OR = math.exp(lr.params.iloc[1])
                    except:
                        OR = 'NA'
                    try:
                        L95 = math.exp(lr.conf_int().iloc[1, 0])
                    except:
                        L95 = 'NA'
                    try:
                        U95 = math.exp(lr.conf_int().iloc[1, 1])
                    except:
                        U95 = 'NA'
                except:
                    p = 'NA'
        ...
        if allele not in excludeAlleles:
            ...
            if f12 > freq:
                ...
                try:
                    ...
                    L95 = lr.conf_int().iloc[1, 0]
                    U95 = lr.conf_int().iloc[1, 1]
                except:
                    p = 'NA'
                    beta = 'NA'
                    L95 = 'NA'
...
##########################  log infor  ##################################################################
print("@-------------------------------------------------------------@")
print("|       PyHLA       |       v3.0      |      08 Apr 2026      |")
print("|-------------------------------------------------------------|")
print("|  (C) 2026 Felix Yanhui Fan, GNU General Public License, v2  |")
print("|-------------------------------------------------------------|")
